...
cv2.imwrite("/binarization1.jpg", gray_image)

thresh,im_bw=cv2.threshold(gray_image,100,230,cv2.THRESH_BINARY)
cv2.imwrite("/thresh1.jpg", im_bw)
...
cv2.imwrite("/noise1.jpg", no_noise)
...
preprocessed_image = cv2.imread("/eroded.jpg", cv2.IMREAD_GRAYSCALE)
...
cv2.imwrite("/pre.jpg", preprocessed_image)
...
with open(".venv/extracted_dates.json", "w") as json_file:
